...
export default function SearchPage() {
  ...
  useEffect(() => {
    const loadPolicies = async () => {
      ...
      try {
        setLoading(true);
        console.log('🔵 검색 시작:', { query, selectedRegion, selectedCategory });
        ...
        console.log('🔍 검색 API 응답 전체:', JSON.stringify(data, null, 2));
        console.log('📊 policies 개수:', data.policies?.length || 0);
        console.log('📊 total_count:', data.total_count);
        console.log('📊 policies 타입:', Array.isArray(data.policies) ? '배열' : typeof data.policies);
        console.log('📊 policies 샘플:', data.policies?.slice(0, 2));
        
        if (!data.policies || data.policies.length === 0) {
          console.warn('⚠️ policies 배열이 비어있습니다!', {
        ...
        console.log('✅ setSearchResult 호출 완료');
      } catch (error: any) {
