...
from .workflows import run_qa_workflow, run_search_workflow
...
class AgentController:
    ...
    def run_search(
        ...
        """
        Search 워크플로우 실행 (LangGraph 기반 고도화된 검색)

        워크플로우 흐름:
        1. Query Understanding: LLM으로 쿼리 분석 (의도, 키워드, 필터)
        2. Retrieve: Qdrant + MySQL 하이브리드 검색
        3. Check Sufficiency: top_1 유사도 0.65 기준 충분성 검사
        4. [조건부] Web Search: 불충분 시 Tavily 웹 검색
        5. Summarize: 결과 요약 및 최종 정책 리스트 구성
