...
class PolicyRepository:
    ...
    def search(
        ...
        try:
            ...
            if query:
                # Search in 주요 텍스트 필드 (정책명, 개요, 대상, 지원 내용 등)
                # LIKE 조건을 넓게 잡아 '창업' 같은 키워드가 최대한 많이 매칭되도록 합니다.
                keyword = f"%{query}%"
                search_filter = or_(
                    Policy.program_name.like(keyword),
                    Policy.program_overview.like(keyword),
                    Policy.apply_target.like(keyword),
                    Policy.support_description.like(keyword),
                )
